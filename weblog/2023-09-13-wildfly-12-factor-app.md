...
Using the [WildFly Maven Plugin](https://github.com/wildfly/wildfly-maven-plugin) to provision WildFly, ensure that all your [12-factor's Dependencies](https://12factor.net/dependencies) are explicitly declared. Wheneve a new version of WildFly is released, you can be notified with something like Dependabot and automatically test your application with this new release.
...
Using [12-factor's Backing Services](https://12factor.net/backing-services) is straightforward and we are able for example to connect to a database with a few env vars representing its URL and credentials with the [datasources feature pack](https://github.com/wildfly-extras/wildfly-datasources-galleon-pack).
